...
/**
 * Standalone {@link LightBatisSession} on plain JDBC, with a thread-bound
 * transaction scope (design §05 of the build plan).
 *
 ...
 * method body, while {@link #begin()}/{@link LightBatisTx#close()} nest
 ...
 */
public final class JdbcLightBatisSession implements LightBatisSession {
    ...
    public void release(Connection c) {
        ...
        if (scope != null && scope.connection == c) {
            return; // the transaction owns it; LightBatisTx.close() will clean up
        }
        try {
            c.close();
        } catch (SQLException e) {
    ...
    public RuntimeException translate(SQLException e, String sql) {
        ...
        return new LightBatisException(message.toString(), sql, e);
    }
    ...
    public LightBatisTx begin(boolean readOnly) {
        ...
        if (existing != null) {
            existing.depth++;
            return new LightBatisTx(this, existing, false);
        }
        Connection c;
        try {
        ...
        currentTx.set(scope);
        return new LightBatisTx(this, scope, true);
    }
    ...
    /** addSuppressed discipline for cleanup paths (build plan §05, pitfall 3). */
    ...
    /** One thread-bound transaction; shared by every nested LightBatisTx handle. */
