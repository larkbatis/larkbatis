 ...
 * talking to a database (the idea originates in minibatis-poc/Demo.java):
 * ① the final SQL string handed to
...
public final class RecordingDataSource implements DataSource {
    ...
    private final class PreparedStatementHandler implements java.lang.reflect.InvocationHandler {
        ...
        public Object invoke(Object proxyObject, Method method, Object[] args) throws Throwable {
            ...
            switch (name) {
                ...
                case "getGeneratedKeys": return generatedKeyResultSet();
    ...
    /**
     * One row carrying key {@code 1}. A driver asked for generated keys after
     * an INSERT returns a row, and both frameworks assign it to keyProperty;
     * an empty one would mean "the database produced no key", which
     * LarkBatis refuses rather than leaving the id at zero. The value is
     * never compared — this harness records the SQL and the bound parameters,
     * not what came back.
     */
    private static ResultSet generatedKeyResultSet() {
        ...
        return proxy(ResultSet.class, (proxyObject, method, args) -> switch (method.getName()) {
            ...
            // boxed to the accessor's own type: the proxy hands the value
            // straight back, and a Long slot given an Integer is a ClassCastException
            case "getLong" -> 1L;
            case "getInt" -> 1;
            case "getShort" -> (short) 1;
            case "getByte" -> (byte) 1;
            case "getObject", "getBigDecimal" -> java.math.BigDecimal.ONE;
            case "getString" -> "1";
            case "close", "getWarnings" -> null;
            case "isClosed" -> false;
            case "wasNull" -> false;
            case "getType" -> ResultSet.TYPE_FORWARD_ONLY;
            // MyBatis's Jdbc3KeyGenerator reads the metadata to pick a
            // TypeHandler for the key column; LarkBatis knows the type from
            // the setter and never asks.
            case "getMetaData" -> proxy(ResultSetMetaData.class,
                    (p, m, a) -> switch (m.getName()) {
                        ...
                        case "getColumnClassName" -> "java.lang.Long";
                        default -> throw new UnsupportedOperationException(
                                "RecordingDataSource does not stub ResultSetMetaData."
                                        + m.getName());
                    });
            case "toString" -> "GeneratedKeyResultSet";
            default -> throw new UnsupportedOperationException(
